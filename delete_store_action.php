...
<html lang="en">
  ...
  <body>
    ...
    <?php
        ...
        if(isset($_GET['id'])){
            $S_ID=$_GET['id'];
            $query="delete from manager where S_ID='$S_ID'";
            $success=$conn->query($query);
            $query="delete from products where S_ID='$S_ID'";
            $success=$conn->query($query);
            $query="delete from orders where S_ID='$S_ID'";
            $success=$conn->query($query);
            $query="delete from store where S_ID='$S_ID'";
            $success=$conn->query($query);
            if (!$success){
                $msg="Failed to delete Store";
                echo   '<script>
            ...
            unlink("images/Stores/store_".$S_ID.".jpg");
        }
