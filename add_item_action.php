  ...
  <body>
    ...
    <?php
        ...
        if(isset($_POST['submit'])){
            ...
            $msg="Failed to add Store item";
            $query="select * from manager where username='$username'";
            $result = $conn->query($query);
            if($result->num_rows>0){
                $row=$result->fetch_assoc();
                $S_ID=$row['S_ID'];
                $query="insert into products(name,price,description,S_ID) values('$name','$price','$description','$S_ID')";
                ...
                $target_dir = "images/store_items/";
                $image_name = "item_".$product_ID.".jpg";
